...
async def run_attack(chat_id, ip, port, duration, context):
    try:
        process = await asyncio.create_subprocess_shell(
            f"./dark {ip} {port} {duration} 180 900",
...
def get_status(active=False, subscription_expiry=None, server_status="🟢 SERVERS AVAILABLE"):
    """
    Generates a status message with dynamic subscription status, server status, and current time.

    Args:
        active (bool): True if the user has an active subscription, False otherwise.
        subscription_expiry (datetime or None): The expiration date of the subscription, or None if no active subscription.
        server_status (str): The status of the server. Defaults to "🟢 SERVERS AVAILABLE"
    ...
    # Dynamically set subscription status
    subscription_status_symbol = "🟢" if active else "❌"
    
    if subscription_expiry:
        expiry_str = subscription_expiry.strftime("%Y-%m-%d %H:%M:%S IST")
    else:
    ...
    status_message = (
        ...
        f"• Status: {subscription_status_symbol} {'Active' if active else 'Inactive'}\n"
        ...
        "⏳ 𝗖𝗼𝗼𝗹𝗱𝗼𝘄𝗻 𝗦𝘁𝗮𝘁𝘂𝘀:\n"
        "• Status: 🟢 Ready\n"
        f"• Duration: {COOLDOWN_DURATION_MINUTES} minutes per attack\n\n"
        "⏰ 𝗟𝗮𝘀𝘁 𝗨𝗽𝗱𝗮𝘁𝗲𝗱:\n"
        f"• {current_time}\n"
        "━━━━━━━━━━━━━━━"
    )
...
if __name__ == '__main__':
    # Example Usage
    status_inactive = get_status(active=False)
    print("Inactive User Status:\n",status_inactive)

    expiry_date = datetime(2024, 12, 31, 23, 59, 59)
    status_active = get_status(active=True, subscription_expiry=expiry_date)
    print("\nActive User Status:\n",status_active)

    status_unavailable = get_status(active=True, subscription_expiry=expiry_date, server_status="🔴 SERVERS UNAVAILABLE")
    print("\nActive User Status with Server Unavailable:\n", status_unavailable)
